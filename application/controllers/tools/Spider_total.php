...
class spider_total extends CI_Controller {
	...
	{
		//判断权限
		$this->public_section->is_access('tools/spider_total');
		
		$this->lang->load('tools/spider_total');
		//header部分
		...
		$this->load->view('tools/spider_total',$data);
	...
	public function truncate_report(){
		...
		if($result){
			if($result['true']){
				$this->session->set_flashdata('setting_success', '清空'.arrayToString($result['true']).'成功！');
