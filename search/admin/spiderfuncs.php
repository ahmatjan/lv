	...
	if (!$fp) {
		...
		if (preg_match("/HTTP/[0-9.]+ (([0-9])[0-9]{2})/", $answer, $regs)) {
			...
			if ($httpcode <> 2 && $httpcode <> 3) {
				$status['state'] = "无法访问: http $full_httpcode";
		...
		if ($linkstate <> "Unreachable") {
			while ($answer) {
				...
				if (preg_match("/Location: *([^\n\r ]+)/", $answer, $regs) && $httpcode == 3 && $full_httpcode != 302) {
					$status['path'] = $regs[1];
					$status['state'] = "迁移: http $full_httpcode";
					fclose($fp);
					return $status;
				}
			...
			if (preg_match("/Content-Type: *([a-z\/.-]*)/i", $content, $regs)) {
				if ($regs[1] == 'text/html' || $regs[1] == 'text/' || $regs[1] == 'text/plain') {
					...
					$status['state'] = "不是文本或网页";
				}

			} else
				if ($socket_status['timed_out'] == 1) {
					$status['state'] = "超时（服务器无应答）";
...
/*
阅读robots.txt文件在服务器上，找到任何禁止的文件/文件夹
*/
...
/*
从URL中删除文件的部分（建立从URL的链接，并给出相对路径）
*/
...
function get_head_data($file) {
	...
	if ($headdata != "") {
		...
		//utf-8的话过滤英文和字符
		preg_match_all("/[\x{4e00}-\x{9fa5}]+/u", $description, $chinese);
		$description = implode("", $chinese[0]);
		$description = str_replace('拼途网','旅行兔网',$description);
		$description = str_replace('拼途','旅行兔',$description);
...
function get_travel_data($file){
	$travel_data="";
	$html = new simple_html_dom();
	$html->load($file);
	$title=$html->find('h2[id=tgTitle]',0)->plaintext;
	$title=preg_replace('/s(?=s)/', '', $title);
	$title=str_replace('-拼途网','',$title);
	$title=str_replace('拼途网','',$title);
	$title=str_replace('&nbsp;','',$title);
	$title=str_replace('拼途','',$title);
	$travel_data['tra_title'] = addslashes($title);//结伴标题addslashes($title);
	
	//头像
	if(!empty($html->find('div[class=colBoxL]',0))){
		$user_img=$html->find('div[class=colBoxL]',0)->find('img');

		if(isset($user_img)){
			foreach ($user_img as $v){
				$travel_data['user_img']=$v->src;
			}
		}
	}
	...
	//用户名
	$travel_data['user_name']=$html->find('a[class=cuName]',0)->plaintext;
	
	$travel_data['user_description']=$html->find('dd[class=sfB]',0)->plaintext;
	
	//标签主题
	$lable=$html->find('div[class=icLabel]',0)->plaintext;
	$travel_data['lable']=preg_replace("/\s/","",$lable);
	//出发日期
	if(!empty($html->find('div[class=timePlace]',0))){
		$travel_data['godate']=$html->find('div[class=timePlace]',0)->find('p',0)->plaintext;
	}
	
	//路线
	if(!empty($html->find('div[class=timePlace]',0))){
		$route=$html->find('div[class=timePlace]',0)->find('p',1)->plaintext;
		$travel_data['route']=preg_replace("/\s/","",$route);
	}
	
	//内容
	$content=$html->find('div[class=dcText]',0)->plaintext;
	$content=str_replace('-拼途网','-旅行兔网',$content);
	$content=str_replace('拼途网','旅行兔网',$content);
	$content=str_replace('&nbsp;','',$content);
	$content=str_replace('拼途','旅行兔',$content);
	$content=str_replace('pintour','lvxingto',$content);
	$content=preg_replace("/\s/","",$content);//正文内容去空格，换行
	$travel_data['content'] = addslashes($content);//正文内容;pintour.com
	/*
	$content=preg_replace("/<a[^>]*>(.*)<\/a>/isU",'${1}',$content);//去a标签
	*/
	
	//正文图片
	if(!empty($html->find('div[class=dcText]',0))){
		$rand=rand(0,5);
		$travel_data['content_img']=$html->find('div[class=dcText]',0)->find('img',$rand)->src;	
	}
	
	//发布日期
	if(!empty($html->find('p[class=llTextL]',0))){
		$adddate=$html->find('p[class=llTextL]',0)->find('span',0)->innertext;
		$travel_data['adddate']=preg_replace("/<(span.*?)>(.*?)<(\/span.*?)>/si","",$adddate); //过滤<span>及其中间的内容	
	}
	
	//获取评论
	if(!empty($html->find('ul[class=reply]'))){
		foreach($html->find('ul[class=reply]') as $ul) 
		{
		       foreach($ul->find('li') as $li)
		       {
				 	$c['reviews_user_img'][]=$li->find('img',0)->src;//回复用户头像
				 	
				 	$c['review_username'][]=$li->find('div[class=icName]',0)->find('a',0)->plaintext;//回复用户的名称
				 	$c['review_usersex'][]=$li->find('div[class=icName]',0)->find('span',0)->plaintext;//回复用户的性别
					
					$re_content=$li->find('p[class=mateText]',0)->plaintext;
					$re_content=str_replace('-拼途网','-旅行兔网',$re_content);
					$re_content=str_replace('拼途网','旅行兔网',$re_content);
					$re_content=str_replace('&nbsp;','',$re_content);
					$re_content=str_replace('拼途','旅行兔',$re_content);
					$c['review_content'][]=str_replace('pintour','lvxingto',$re_content);
					
					$c['rmTime'][]=$li->find('p[class=rmTime]',0)->find('span',0)->plaintext;//回复时间
		       }
		}
		$travel_data['reviews']=$c;	
		}
...
function clean_file($file, $url, $type) {
	...
	$html->load($file);
	//随机提取一张图片
	if(!empty($html->find('body',0))){
		$rand=rand(3,8);
		$full_img=$html->find('body',0)->find('img',$rand)->src;	
	}
	$html->clear();
	...
	$data['travel_data']=get_travel_data($file);
	//抓结伴
	$data['tra_title']=$travel_data['tra_title'];
	$data['user_img']=$travel_data['user_img'];
	//抓结伴
	$regs = Array ();
	...
	$file = preg_replace_callback('~&#x([0-9a-f]+);~ei', 'chr(hexdec("\\1"))', $file);
    //$file = preg_replace('~&#([0-9]+);~e', 'chr("\\1")', $file);
    $file = preg_replace_callback('~&#([0-9]+);~e', 'chr("\\1")', $file);
