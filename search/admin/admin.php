...
<div id="admin"> 
	<div id="tabs">
		...
		<li><a href="admin.php?f=2" id="<?php print $site_funcs[$f]?>">网站</a>  </li>
		...
		<li><a href="admin.php?f=clean" id="<?php print $clean_funcs[$f]?>">清理数据表</a> </li>
		...
		<li><a href="admin.php?f=24" id="default">退出</a></li>
		</ul>
	</div>
	<div id="main">
...
?>
	   <br/><center><table><tr><td valign=top align=center colspan=2><b>上一级: <?php print "<a href=admin.php?f=add_cat&parent=$par2num>$par2</a> >".stripslashes($par)?></b></td></tr>
...
<?php 
	...
	function addsiteform() {
		?>
		<div id="submenu"><center><b>添加一个网站</b></center></div>
	...
	function editsiteform($site_id) {
		...
		?>
					<div id="submenu"><center><b>修改网站</b></center>
			</div>
			<br/><div align=center><center><table>
			<form action=admin.php method=post>
			...
			<tr><td><b>标题:</b></td><td></td><td> <input placeholder="抓取标题只在后台显示..." type=text name=title value=<?php print  "\"".stripslashes($row['title'])."\""?> size=60></td></tr>
			<tr><td><b>简短描述:</b></td><td></td><td><textarea placeholder="描述只作为名称在后台显示..." name=short_desc cols=45 rows=3 wrap><?php print stripslashes($row['short_desc'])?></textarea></td></tr>
			<tr><td><b>抓取选项:</b></td><td></td><td><input type="radio" name="soption" value="full" <?php print $fullchecked;?>> 全部<br/>
			<input type="radio" name="soption" value="level" <?php print $depthchecked;?>>抓取深度: <input type="text" name="depth" size="2" value="<?php print $depth;?>"><br/>
			<input type="checkbox" name="domaincb" value="1" <?php print $domainchecked;?>> 蜘蛛可以离开域
			</td></tr>			
			<tr><td><b>URL必须包括:</b></td><td></td><td><textarea placeholder="用空格分割..." name=in cols=45 rows=2 wrap="virtual"><?php print $row['required'];?></textarea></td></tr>
			<tr><td><b>URL必须不包括:</b></td><td></td><td><textarea placeholder="用空格分割..." name=out cols=45 rows=2 wrap="virtual"><?php print $row['disallowed'];?></textarea></td></tr>
			
			<tr><td>分类:</td><td></td><td>
			<?php  walk_through_cats(0, 0, $site_id);?></td></tr>
			<tr><td></td><td></td><td><input type="submit"  id="submit"  value="确定"></td></tr></form></table></center></div>
		<?php 
		}
		...
		function editsite ($site_id, $url, $title, $short_desc, $depth, $required, $disallowed, $domaincb,  $cat) {
			...
			If (!mysql_error()) {
				return "<br/><center><b>网站已更新！</b></center>" ;
			} else {
				return mysql_error();
			}
		}
	...
	function editcat ($cat_id, $category) {
		...
		mysql_query($qry);
		if (!mysql_error())	{
			return "<br/><center><b>分类已更新</b></center>";
		} else {
			return mysql_error();
		}
	}
	...
	function showsites($message) {
		...
		<div id='submenu'>
		 <ul>
		  <li><a href='admin.php?f=add_site'>添加网站</a> </li>
		  <?php 
			if (mysql_num_rows($result) > 0) {
				?>
				<li><a href='spider.php?all=1'>显示所有网站</a></li>
				<?php 
			}
			?>
		 </ul>
		...
		if (mysql_num_rows($result) > 0) {
			...
			<tr class=\"grey\"><td align=\"center\"><b>网站名</b></td><td align=\"center\"><b>URL地址</b></td><td align=\"center\"><b>最后抓取</b></td><td colspan=4></td></tr>\n";
		} else {
			?><center><p><b>欢迎来到【旅行兔】搜索管理后台！<br><br>从子菜单“添加网站”添加一个新的网站，或“索引”直接进入索引部分。</b></p></center><?php 
		}
		$class = "grey";
		while ($row=mysql_fetch_array($result))	{
			if ($row['indexdate']=='') {
				$indexstatus="<font color=\"red\">Not indexed</font>";
				$indexoption="<a href=\"admin.php?f=index&url=$row[url]\">抓取</a>";
			} else {
				$site_id = $row['site_id'];
	...
	function addsite ($url, $title, $short_desc, $cat) {
		...
		if ($rows==0 ) {
			...
			If (!mysql_error())	{
				$message =  "<br/><center><b>网站添加成功</b></center>" ;
			} else {
				$message = mysql_error();
			}

		} else {
			$message = "<center><b>网站已经存在</b></center>";
		}
		return $message;
	}
	...
	function indexscreen ($url, $reindex) {
		...
		<div id="submenu">
			<ul>
				...
				if ($_SESSION['index_advanced']==1){
					...
					print "<a href='admin.php?f=index&adv=1&url=$advurl'>打开高级选项</a>";
				}
		...
		<tr><td><b>地址:</b></td><td> <input type="text" name="url" size="48" value=<?php print "\"$url\"";?>></td></tr>
		<tr><td><b>抓取选项:</b></td><td>
		<input type="radio" name="soption" value="full" <?php print $fullchecked;?>> 全部<br/>
		<input type="radio" name="soption" value="level" <?php print $levelchecked;?>>抓取深度: <input type="text" name="maxlevel" size="2" value="<?php print $spider_depth;?>"><br/>
		...
		if ($_SESSION['index_advanced']==1){
			...
			<tr><td></td><td><input type="checkbox" name="domaincb" value="1" <?php print $checkcan;?>> 蜘蛛可以离开域 <!--a href="javascript:;" onClick="window.open('hmm','newWindow','width=300,height=300,left=600,top=200,resizable');" >?</a--><br/></td></tr>
			<tr><td><b>URL必须包括:</b></td><td><textarea name=in cols=35 rows=2 wrap="virtual"><?php print $must;?></textarea></td></tr>
			<tr><td><b>URL必须不包括:</b></td><td><textarea name=out cols=35 rows=2 wrap="virtual"><?php print $mustnot;?></textarea></td></tr>
			<?php 
		}
		?>

		<tr><td></td><td><input type="submit" id="submit" value="开始抓取"></td></tr>
		</form></table></div>
		<?php 
	}
	...
	function siteScreen($site_id, $message)  {
		...
		if ($row['indexdate']=='') {
			$indexstatus="<font color=\"red\">没有抓取</font>";
			$indexoption="<a href=\"admin.php?f=index&url=$url\">抓取</a>";
		} else {
			$site_id = $row['site_id'];
			$result2 = mysql_query("SELECT site_id from ".$mysql_table_prefix."pending where site_id =$site_id");
			echo mysql_error();			
			...
			if ($row2['site_id'] == $row['site_id']) {
				$indexstatus = "Unfinished";
				$indexoption="<a href=\"admin.php?f=index&url=$url\">继续抓取</a>";

			} else {
				$indexstatus = $row['indexdate'];
				$indexoption="<a href=\"admin.php?f=index&url=$url&reindex=1\">重新抓取</a>";
			}
		}
		?>
		...
		<table cellpadding="3" cellspacing="0">
			...
			<tr>
			  ...
			  <tr>
				<td class="grey" valign="top" align="left">描述:</td>
				<td width="80%" class="white"  align="left"><?php print stripslashes($row['short_desc']);?></td>
			  </tr>
			  <tr>
				<td class="grey" valign="top" align="left">最后抓取:</td>
		...
		<div id= "vertmenu">
		<ul>
		 <li><a href=admin.php?f=edit_site&site_id=<?php print  $row['site_id']?>>修改</a></li>
		<li><?php print $indexoption?></li>
		<li><a href=admin.php?f=21&site_id=<?php print  $row['site_id']?>>浏览页面</a></li>
		<li><a href=admin.php?f=5&site_id=<?php print  $row['site_id'];?> onclick="return confirm('你确定要删除吗？删除后不能恢复！')">删除</a></li>
	...
	function browsePages($site_id, $start, $filter, $per_page) {
		...
		<br/>
		<center>
		<b>来自网站的页面<a href="admin.php?f=20&site_id=<?php  print $site_id?>"><?php print $url;?></a></b><br/>
		<p>
		<form action="admin.php" method="post">
		每页面网址: <input type="text" name="per_page" size="3" value="<?php print $per_page;?>"> 
		Url 包含: <input type="text" name="filter" size="15" value="<?php print $filter;?>"> 
	...
	function cleanForm () {
		...
		<br/><div align="center">
		<table cellspacing ="0" cellpadding="0" class="darkgrey"><tr><td align="left"><table cellpadding="3" cellspacing = "1"  width="100%"><tr class="grey"  ><td align="left"><a href="admin.php?f=15" id="small_button">清理关键词</a> 
		 </td><td align="left"> 删除不与任何一个环节相关的所有关键词.</td></tr>
		<tr class="grey"  ><td align="left"><a href="admin.php?f=16" id="small_button">清理链接</a>
		</td><td align="left"> 删除不与任何网站相关联的所有链接。</td></tr>
		<tr class="grey"  ><td align="left"><a href="admin.php?f=17" id="small_button">清除临时表 </a>
		</td><td align="left"> <?php print $temp;?> 在临时表中的项目。</td></tr>
		<tr class="grey"  ><td align="left"><a href="admin.php?f=23" id="small_button">清理搜索记录</a> 
		</td><td align="left"><?php print $log;?>在搜索日志项。
	...
	function statisticsForm ($type) {
		...
		<div id='submenu'>
		<ul>
		<li><a href="admin.php?f=statistics&type=keywords">热门关键词</a></li>
		<li><a href="admin.php?f=statistics&type=pages">访问最多页面</a></li>
		<li><a href="admin.php?f=statistics&type=top_searches">最热门搜索</a></li>
		...
		<?php 
			...
			if ($type=='keywords') {
				$class = "grey";
				print "<br/><div align=\"center\"><table cellspacing =\"0\" cellpadding=\"0\" class=\"darkgrey\"><tr><td><table cellpadding=\"3\" cellspacing = \"1\"><tr  class=\"grey\"><td><b>关键词</b></td><td><b>出现次数</b></td></tr>";
				for ($i=0;$i<=15; $i++) {
					$char = dechex($i);
			...
			if ($type=='top_searches') {
				$class = "grey";
				print "<br/><div align=\"center\"><table cellspacing =\"0\" cellpadding=\"0\" class=\"darkgrey\"><tr><td><table cellpadding=\"3\" cellspacing = \"1\"><tr  class=\"grey\"><td><b>搜索关键词</b></td><td><b>统计</b></td><td><b> 平均结果</b></td><td><b>最近搜索</b></td></tr>";
			...
			if ($type=='log') {
				$class = "grey";
				print "<br/><div align=\"center\"><table cellspacing =\"0\" cellpadding=\"0\" class=\"darkgrey\"><tr><td><table cellpadding=\"3\" cellspacing = \"1\"><tr  class=\"grey\"><td align=\"center\"><b>搜索关键词</b></td><td align=\"center\"><b>结果</b></td><td align=\"center\"><b>查询时间</b></td><td align=\"center\"><b>响应时间</b></td></tr>";
	...
	$stats = getStatistics();
	print "<br/><br/>	<center>当前数据库: ".$stats['sites']." 个网站 | ".$stats['links']." 个链接 | ".$stats['categories']." 个类别 | ".$stats['keywords']." 个关键词.<br/><br/></center>\n";
